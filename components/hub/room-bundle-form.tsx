...
export function RoomBundleForm({ defaultPlace }: { defaultPlace: string }) {
  ...
  const addItem = (name: string) => {
    ...
    setItems((current) => [
      ...current,
      {
        key: Math.max(-1, ...current.map((i) => i.key)) + 1,
        name: value,
        condition: "good",
      },
    ]);
  ...
  const unused = COMMON_ITEMS.filter(
    (name) => !items.some((i) => i.name.toLowerCase() === name.toLowerCase()),
  );
  ...
  if (listed !== null) {
    return (
      <div
        role="status"
        className="rounded-2 border border-rule-strong bg-paper-raised px-5 py-5"
      >
        <p className="t-title text-[22px]">Listed.</p>
        <p className="mt-1">
          Your room is on the board as one bundle with {listed} items. Incoming
          students with matching lists get told right away.
        </p>
  ...
  return (
    ...
    >
      <Field
        ...
        hint="Only the street shows publicly. The unit number goes on the confirmation."
        htmlFor="room"
      >
        <input
          id="room"
      ...
      <fieldset>
        ...
        <p className="mt-1 text-[13px] text-ink-2">
          Tap to add. You can rename anything.
        </p>
        ...
        <ul className="mt-4 border-t border-rule">
          {items.map((item, i) => (
            ...
            >
              <span className="data w-6 shrink-0 text-[13px] text-ink-2">
                {i + 1}
              </span>
              <input
                aria-label={`Item ${i + 1}`}
                ...
                onChange={(e) =>
                  setItems((current) =>
                    current.map((x) =>
                      x.key === item.key ? { ...x, name: e.target.value } : x,
                    ),
                  )
                }
        ...
        {items.length === 0 && (
          <p className="mt-2 text-[13px] font-semibold">
            Add at least one item to list your room.
          </p>
        )}
      </fieldset>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <Field
          label={free ? "Price" : "Price for everything"}
        ...
        >
          {free ? (
            ...
            <div className="relative">
              <span className="data pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-ink-2">
                $
              </span>
              <input
                id="bundle-price"
        ...
        <Field
          label="Gone by"
          hint="Usually the morning you leave."
          htmlFor="room-deadline"
        >
          <input
            id="room-deadline"
            type="datetime-local"
