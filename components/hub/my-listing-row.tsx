...
export function MyListingRow({ listing }: { listing: MyListing }) {
  ...
  return (
    <li className="relative border-b border-rule">
      <span
        aria-hidden="true"
        className={`absolute inset-y-0 left-0 w-0.5 ${claimed ? "bg-ink-3" : "bg-ink"}`}
      />
      <Link
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="t-eyebrow mb-1 text-ink-2">{claimed ? "Claimed · pickup pending" : "Live on the board"}</p>
